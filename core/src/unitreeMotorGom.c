...
bool initMotoCmdGom(struct MotorCmdGom * cmd,const unsigned short *ID, const unsigned short *MODE, const float* T,const float* W,const float* POS,const float* K_P,const float* K_W){
    ...
    if(*ID > 15 || *ID < 0) return false;
    if(*T < -127 || *T > 127) return false;
    if(*W < -804 || *W > 804) return false;
    if(*POS < -411774 || *POS > 411774) return false;
    if(*K_P < 0 || *K_P > 25.5) return false;
    if(*K_W < 0 || *K_W > 25.5) return false;
    cmd->head = 0xEEFE; 
